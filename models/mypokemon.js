...
class MyPokemon {
    ...
    static async getListMyPokemons() {
        const [rows] = await connection.execute('SELECT users_pokemons.id, pokemons.id pokemon_id, pokemons.name, pokemons.avatar, pokemons.type, pokemons.weaknesses, pokemons.description FROM users_pokemons JOIN pokemons ON users_pokemons.pokemon_id = pokemons.id');
        return rows;
    }
